...
export default function WorkspaceHome({ matchId }: { matchId?: string }) {
  ...
  useEffect(() => {
    if (matchId) {
      fetchReport(periodStart, periodEnd, matchId);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_2px_20px_rgb(0,0,0,0.03)] p-6">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-end mb-6">
          <div className="w-full md:w-auto">
            <h2 className="text-lg font-bold text-slate-900">Temps de travail et tâches</h2>
            <p className="text-xs text-slate-400 mt-1">Consultez vos statistiques de travail sur la période sélectionnée.</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[2rem] p-12 border border-slate-100 shadow-[0_2px_20px_rgb(0,0,0,0.03)] flex flex-col items-center text-center">
          ...
          <p className="text-slate-500">Vous n'avez pas enregistré de temps sur cette période.</p>
        </div>
      ) : (
        <>
          {/* Graphiques */}
