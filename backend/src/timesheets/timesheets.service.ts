...
import { TimerStatus, TimeEntry } from '@prisma/client';
import {
  CreateTimesheetDto,
  UpdateTimesheetDto,
  StartTimerDto,
...
export class TimesheetsService {
  ...
  private async checkIfEditable(entryId: string, userId: string): Promise<TimeEntry> {
  ...
  async resumeTimer(userId: string): Promise<TimeEntry> {
    const activeTimer = await this.getActiveTimer(userId);
  ...
  async getActiveTimer(userId: string): Promise<TimeEntry> {
    const activeTimer = await this.prisma.timeEntry.findFirst({
      where: {
        user_id: userId,
        status: { in: [TimerStatus.running, TimerStatus.paused] },
  ...
  async createEntry(userId: string, createDto: CreateTimesheetDto): Promise<TimeEntry> {
    const now = new Date();
    const limitDate = new Date();
    limitDate.setDate(now.getDate() - EDITABLE_DAYS);
  ...
  async updateEntry(userId: string, id: string, updateDto: UpdateTimesheetDto): Promise<TimeEntry> {
  ...
  async getEntries(userId: string, matchId?: string, startDate?: Date, endDate?: Date): Promise<TimeEntry[]> {
